...
if ( ! class_exists( 'Dev_Tool_Functions' ) ) {

    class Dev_Tool_Functions {
        ...
        function developer_tool_update_settings() {
            ...
            if ( isset( $_POST['developer-tool-settings'] ) && ! empty( $_POST['developer-tool-settings'] ) ) {
                ...
                $fileContent = $this->read_debug_log_file( $fileName );
                ...
                if ( $this->write_debug_log_file( $fileContent, $fileName ) ) { ?>
        ...
        function download_wp_config_file( $path ) {
            $content = $this->read_debug_log_file( $path );
        ...
        function developer_tool_config() {
            ...
            if( is_super_admin() ){
                $this->download_wp_config_file( 'wp-config.php' );
            }
        }
    }

    Dev_Tool_Functions::get_instance();
}
 ...
 * @return object
 */
function dev_tool_helper() {

    return Dev_Tool_Functions::get_instance();
}
